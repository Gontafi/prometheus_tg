...
var (
	//whURL         = flag.String("webhook.url", "https://discordapp.com/api/webhooks/1143121677467795466/TuS8W5WSwp0rK8Sd3It-SdWmIwnoEz12vRYR239T4QfMO6zOjDSkJxccQbNpq5YEPbXn", "Discord WebHook URL.")
...
var ipHost map[string]interface{}
var chatIDs []int64

const fileName = "hostname.json"
const chatIDFileName = "chat_id.json"
...
func main() {
	...
	chatIDsfile, err := os.Open(chatIDFileName)
	if err != nil {
		log.Fatal(err)
	}
	chatIDStat, err := chatIDsfile.Stat()
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	ci := make([]byte, chatIDStat.Size())
	_, err = bufio.NewReader(chatIDsfile).Read(ci)
	if err != nil && err != io.EOF {
		fmt.Println(err)
		return
	}

	err = sonic.Unmarshal(bs, &ipHost)
	if err != nil {
		log.Fatal(err)
	}
	...
	err = json.Unmarshal(ci, &ChatData)
	if err != nil {
		log.Fatal(err)
	}
...
func AlertFormatTemplate(alerts Alerts) string {
	for _, val := range alerts.GroupLabels {
		_, ok := ipHost[val.(string)]
		if ok {
			alerts.GroupLabels["instance"] = ipHost[val.(string)]
		}
	}
	for _, a := range alerts.Alerts {
		for _, val := range a.Labels {
			_, ok := ipHost[val.(string)]
			if ok {
				a.Labels["instance"] = ipHost[val.(string)]
			}
		}
	}
	for _, val := range alerts.CommonLabels {
		_, ok := ipHost[val.(string)]
		if ok {
			alerts.CommonLabels["instance"] = ipHost[val.(string)]
		}
	}
...
func POST_Handling(ctx *fiber.Ctx) error {
	var msgtext string
	var alerts Alerts

	err := ctx.BodyParser(&alerts)
	if err != nil {
